...
const handleDelete = async (index: number, row: MenuModel) => {
    ...
    ElMessageBox.confirm(
        ...
        .then(() => {
            ...
            // const res = await delEmployee(row.id) as any as boolean
            const res = delMenu(row.id) as any as boolean
            if (res) {
                ElMessage({
                    message: '删除成功！',
                    type: 'success',
...
const Del = async () => {
    ...
    if (arr.length == 0) {
        ...
        ElMessageBox.confirm(
            ...
            .then(() => {
                const res = batchDelMenu(ids) as any as boolean
                if (res) {
                    ElMessage({
                        message: '删除成功！',
                        type: 'success',
...
<template>
    <el-card class="box-card">
        ...
        <el-table v-loading="loading" :data="tableData" style="width: 100%" ref="multipleTableRef"  row-key="id" >
            ...
            <el-table-column label="菜单路由" width="200">
                <template #default="scope">
                    <div>{{ scope.row.index }}</div>
                </template>
            </el-table-column>
            <el-table-column label="文件路径" width="350">
                <template #default="scope">
