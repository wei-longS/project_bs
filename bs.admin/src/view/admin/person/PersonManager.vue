...
const handleDelete = async (index: number, row: PersonModel) => {
    ...
    ElMessageBox.confirm(
        ...
        .then(() => {
            ...
            const res = delUsers(row.id) as any as boolean
            if (res) {
                ElMessage({
                    message: '删除成功！',
                    type: 'success',
...
const Del = async () => {
    ...
    if (arr.length == 0) {
        ...
        ElMessageBox.confirm(
            ...
            .then(() => {
                const res = batchDelUsers(ids) as any as boolean
                if (res) {
                    ElMessage({
                        message: '删除成功！',
                        type: 'success',
...
<template>
    <el-card class="box-card">
        ...
        <el-table v-loading="loading" :data="tableData" style="width: 100%" ref="multipleTableRef">
